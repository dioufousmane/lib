...
function legend() {
    ...
    overlays.getLayers().getArray().slice().forEach(layer => {
        ...
        element.appendChild(head);
        var img = new Image();
        img.src = "http://localhost:80/geoserver/wms?REQUEST=GetLegendGraphic&VERSION=1.0.0&FORMAT=image/png&WIDTH=20&HEIGHT=20&LAYER=" + layer.get('title');
...
$(document).ready(function() {
    $.ajax({
        type: "GET",
        url: "http://localhost:80/geoserver/wfs?request=getCapabilities",
        dataType: "xml",
...
$(function() {
    $("#layer").change(function() {
        ...
        $(document).ready(function() {
            $.ajax({
                type: "GET",
                url: "http://localhost:80/geoserver/wfs?service=WFS&request=DescribeFeatureType&version=1.1.0&typeName=" + value_layer,
                dataType: "xml",
                success: function(xml) {
...
function query() {
    ...
    var url = "http://localhost:80/geoserver/ows?service=WFS&version=1.0.0&request=GetFeature&typeName=" + value_layer + "&CQL_FILTER=" + value_attribute + "%20" + value_operator + "%20" + value_txt + "&outputFormat=application/json"
    //console.log(url);
...
function wms_layers() {
    ...
    $(document).ready(function() {
        $.ajax({
            type: "GET",
            url: "http://localhost:80/geoserver/wms?request=getCapabilities",
            dataType: "xml",
            success: function(xml) {
...
function add_layer() {
    ...
    var url = 'http://localhost:80/geoserver/wms?request=getCapabilities';
...
function info() {
    if (document.getElementById("info_btn").innerHTML == "☰ Activer l'indentification") {

        document.getElementById("info_btn").innerHTML = "☰ Désctiver l'indentification";
        ...
        document.getElementById("info_btn").innerHTML = "☰ Activer l'indentification";
...
function clear_all() {
    ...
    document.getElementById("query_panel_btn").innerHTML = "☰ Lancer une requête";
    ...
    document.getElementById("legend_btn").innerHTML = "☰ Montrer la légende";
    ...
    document.getElementById("info_btn").innerHTML = "☰ Activer l'interrogation";
    document.getElementById("info_btn").setAttribute("class", "btn btn-success btn-sm");
    map.updateSize();
...
function show_hide_querypanel() {
    ...
    if (document.getElementById("query_tab").style.visibility == "hidden") {
        ...
        document.getElementById("query_panel_btn").innerHTML = "☰ Fermer le panneau";
    ...
    if (value !== 'None') {
		...
		draw1 = new ol.interaction.Draw({
            ...
            draw1.on('drawend', function(evt) {
                ...
                var url = "http://localhost:80/geoserver/wfs?request=GetFeature&version=1.0.0&typeName=" + value_layer + "&outputFormat=json&cql_filter=INTERSECTS(the_geom," + wkt + ")";
                //alert(url);
